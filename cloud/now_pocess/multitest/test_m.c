...
int main(void)
{
    ...
    if( child == P_MAX ){			//親プロセスはすべての子プロセスの終了を待つ
      ...
      if(child == 0){
        port_comp = 50140; //子プロセス1のポート番号
      }else if(child == 1){
        port_comp = 50340; //子プロセス2のポート番号
      }
              ...
              while( ptr = strtok(NULL, ",") ){
                if(i == 0 && ptr != NULL){
                  ...
                  if(count % 3 == 0){
                    ...
                    PROC_DEB(child);
                    printf("value%10d -> %10d\n", print_counter,VALUE_BUFF[value_counter1]);
                    value_counter1++;
                  }else if(count % 3 == 1){
                    VALUE_BUFF2[value_counter2] = atoi(value);
                    PROC_DEB(child);
                    printf("value%10d -> %10d\n", print_counter,VALUE_BUFF2[value_counter2]);
                    value_counter2++;
                  }else if(count % 3 == 2){
                    VALUE_BUFF3[value_counter3] = atoi(value);
                    PROC_DEB(child);
                    printf("value%10d -> %10d\n", print_counter,VALUE_BUFF3[value_counter3]);
                    value_counter3++;
                  }
